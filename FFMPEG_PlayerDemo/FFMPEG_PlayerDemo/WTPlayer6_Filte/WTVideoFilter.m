...
//  FFMPEG_PlayerDemo
//
//  Created by tyler on 5/31/19.
//  Copyright © 2019 ocean. All rights reserved.
//

#import "WTVideoFilter.h"
#include <stdio.h>
...
@interface WTVideoFilter ()

@end

@implementation WTVideoFilter
...
-(int)mainFilteAction {
    ...
    if (avformat_open_input(&ifmt_ctx, device_name, ifmt, &device_param) != 0){
        ...
        printf("Couldn't open input video stream.£®Œﬁ∑®¥Úø™ ‰»Î¡˜£©\n");
        return -1;
    }
    //Set own audio device's name
    if (avformat_open_input(&ifmt_ctx_a, device_name_a, ifmt, &device_param) != 0){
        
        printf("Couldn't open input audio stream.£®Œﬁ∑®¥Úø™ ‰»Î¡˜£©\n");
        return -1;
    }
    ...
    if (videoindex == -1)
    {
        printf("Couldn't find a video stream.£®√ª”–’“µΩ ”∆µ¡˜£©\n");
        return -1;
    }
    ...
    {
        printf("Could not open video codec.£®Œﬁ∑®¥Úø™Ω‚¬Î∆˜£©\n");
        return -1;
    }
    //input audio initialize
    if (avformat_find_stream_info(ifmt_ctx_a, NULL) < 0)
    {
        printf("Couldn't find audio stream information.£®Œﬁ∑®ªÒ»°¡˜–≈œ¢£©\n");
        return -1;
    }
    audioindex = -1;
    ...
    if (audioindex == -1)
    {
        printf("Couldn't find a audio stream.£®√ª”–’“µΩ ”∆µ¡˜£©\n");
        return -1;
    }
    ...
    {
        printf("Could not open audio codec.£®Œﬁ∑®¥Úø™Ω‚¬Î∆˜£©\n");
        return -1;
    }
    ...
    if (!pCodec){
        printf("Can not find output video encoder! (√ª”–’“µΩ∫œ  µƒ±‡¬Î∆˜£°)\n");
        return -1;
    }
    ...
    if (avcodec_open2(pCodecCtx, pCodec, &param) < 0){
        printf("Failed to open output video encoder! (±‡¬Î∆˜¥Úø™ ß∞‹£°)\n");
        return -1;
    }
    ...
    if (!pCodec_a){
        printf("Can not find output audio encoder! (√ª”–’“µΩ∫œ  µƒ±‡¬Î∆˜£°)\n");
        return -1;
    }
    ...
    if (avcodec_open2(pCodecCtx_a, pCodec_a, NULL) < 0){
        printf("Failed to open ouput audio encoder! (±‡¬Î∆˜¥Úø™ ß∞‹£°)\n");
        return -1;
    }
    ...
    if (avio_open(&ofmt_ctx->pb, out_path, AVIO_FLAG_READ_WRITE) < 0){
        printf("Failed to open output file! ( ‰≥ˆŒƒº˛¥Úø™ ß∞‹£°)\n");
        return -1;
    }
    ...
    NSThread *hThread = [[NSThread alloc]initWithTarget:self selector:@selector(MyThreadFunction) object:nil];
    [hThread start];
                    ...
                    while (1) {
                        ...
                        if (picref) {
                            ...
                            if (enc_got_frame == 1){
                                ...
                                int64_t calc_duration = (double)(AV_TIME_BASE)*(1 / av_q2d(r_framerate1));    //ƒ⁄≤ø ±º‰¥¡
                                //Parameters
