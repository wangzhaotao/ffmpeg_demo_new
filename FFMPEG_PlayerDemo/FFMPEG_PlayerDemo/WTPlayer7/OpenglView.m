...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        lineView.backgroundColor=iColor(26, 26, 26, 1);
...
- (BOOL)createFrameAndRenderBuffer:(CAEAGLLayer *)eaglLayer
{
    ...
    int loc = glGetAttribLocation(_program, "position");
    glVertexAttribPointer(loc, 3, GL_FLOAT, 0, 5 * sizeof(GL_FLOAT), (void *)0);
    glEnableVertexAttribArray(loc);
    loc = glGetAttribLocation(_program, "TexCoordIn");
    glVertexAttribPointer(loc, 2, GL_FLOAT, 0, 5 * sizeof(GL_FLOAT), (void *)(3 * sizeof(GL_FLOAT)));
    glEnableVertexAttribArray(loc);
    ...
    {
        iLog(@"创建缓冲区错误 0x%x", glCheckFramebufferStatus(GL_FRAMEBUFFER));
        return NO;
    }
    return YES;
}
...
/**
 ...
 @param h h
 */
-(void)displayYUV420pData:(void *)data width:(NSInteger)w height:(NSInteger)h isKeyFrame:(BOOL)isKeyFrame{
    //TODO: 设置alpha的操作可以在统一修改到组件外部
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.alpha < 1.0) {
    ...
    //以下在合并代码前有的，合并时发现dev上没有就注释了
//    if (self.isNeedClear==YES) {
//        self.isNeedClear=NO;
//        [self clearFrame];
//        [self initFrameAndRenderBuffer];
//    }
//    if (!_onSuperview) { return ; }
    /*
    if (self.window) {
        return;
    }
...
- (void)destroy{
    @synchronized (self) {
        ...
        _glContext = nil;
        NSLog(@"OpenglView destroy");
    }
}

- (void)dealloc{

    iLog(@"OpenglView dealloc");
}
@end
