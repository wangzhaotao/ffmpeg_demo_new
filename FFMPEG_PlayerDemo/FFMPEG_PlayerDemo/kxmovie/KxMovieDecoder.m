...
#pragma mark life
/*
 initialize和load的区别在于：load是只要类所在文件被引用就会被调用，而initialize是在类或者其子类的第一个方法被调用前调用。
...
-(kxMovieError)openAudioStream:(NSInteger)audioStream {
    ...
    if (!audioCodecIsSupported(codecCtx)) {
        id<KxAudioManager> audioManager = [KxAudioManager audioManager];
        NSLog(@"11audio-channels: %d, audio-sample: %f", audioManager.numOutputChannels, audioManager.samplingRate);
        NSLog(@"11codec-channels: %d, codec-fmt: %d, codec-sample: %d", codecCtx->sample_fmt, codecCtx->channels, codecCtx->sample_rate);
