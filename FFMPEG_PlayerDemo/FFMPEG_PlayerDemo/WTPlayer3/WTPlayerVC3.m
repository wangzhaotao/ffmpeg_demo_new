...
#import "WTMediaDecoder.h"
#import "KxMovieGLView.h"
#import "KxMovieDecoder.h"
#import "BCAudioQueuePlay.h"
#import "AACDecoder.h"

@interface WTPlayerVC3 ()
{
    //播放队列
...
//解码
@property (nonatomic, strong) WTMediaDecoder *mediaDecoder;
//opengl渲染界面
...
//
@property (nonatomic, strong) BCAudioQueuePlay *pcmPalyer;
...
+ (id) movieViewControllerWithContentPath: (NSString *) path
    ...
    WTPlayerVC3 *vc = [[WTPlayerVC3 alloc]initWithPath:path];
    return vc;
}
...
-(void)openMediaPath:(NSString*)path {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (!weakSelf.mediaDecoder) {
            weakSelf.mediaDecoder = [[WTMediaDecoder alloc]init];
            [weakSelf.mediaDecoder openMediaWithPath:path];
            
            //回到主线程
            __strong WTPlayerVC3 *strongSelf = weakSelf;
            if (strongSelf) {
...
-(void)pause {
    ...
    self.playing = NO;
    [self enableAudio:NO];
    //[self enableAudioDisplayLink:NO];
}
-(void)play {
    ...
    if (_mediaDecoder.validAudio) {
        ...
        //另外一个方法
        //[self enableAudioDisplayLink:YES];
    }
}
...
-(void)asynDecodeFrames {
    ...
    //
    __weak WTPlayerVC3 *weakSelf = self;
    __weak WTMediaDecoder *weakDecoder = _mediaDecoder;
    ...
    dispatch_async(_playSerialQueue, ^{
        ...
        while (good) {
            good = NO;
            @autoreleasepool {
                __strong WTMediaDecoder *strongDecode = weakDecoder;
...
//#pragma mark 播放音频
//-(void)enableAudioDisplayLink:(BOOL)enable {
//    
//    if (enable) {
//        [self.aacDecoder creatConverter];
//    }
//    
...
//            [self.aacDecoder creatConverter];
//            
//            self.audioDisplayLink=[CADisplayLink displayLinkWithTarget:self selector:@selector(displayAudio)];
//            self.audioDisplayLink.preferredFramesPerSecond=60;
//            [self.audioDisplayLink addToRunLoop:[NSRunLoop currentRunLoop] forMode:NSRunLoopCommonModes];
//        }else{
//            
//            //[self.audioDisplayLink removeFromRunLoop:[NSRunLoop currentRunLoop] forMode:NSRunLoopCommonModes];
//        }
//    });
//    if (!enable) {
...
//            NSLog(@"displayAudio - currentAudioFrame为空");
//            @synchronized(_audioFrames) {
...
//        
//        if (_currentAudioFrame) {
//            NSLog(@"displayAudio - currentAudioFrame不为空");
//            NSData *tmpData = _currentAudioFrame;
//            BOOL isG711Audio = NO;
//            if (isG711Audio) {
//                NSLog(@"displayAudio - G711");
//                //Floodlight，智新SDK中音频采用G.711编解码
//                [weakSelf.pcmPalyer playWithData:tmpData];
//            } else{
//                //Camera采用AAC编码
//                NSLog(@"displayAudio - AAC");
//                NSMutableData *payload = [NSMutableData dataWithData:tmpData];
//                [payload replaceBytesInRange:NSMakeRange(0, 7) withBytes:NULL length:0];
//                [weakSelf.aacDecoder pushData:(uint8_t *)[payload bytes] size:(uint8_t)[payload length] completion:^(NSData *pcmData) {
//                    NSLog(@"displayAudio - PCM");
//                    [weakSelf.pcmPalyer playWithData:pcmData];
//                }];
//            }
...
#pragma mark set/get
- (BCAudioQueuePlay *)pcmPalyer{
    if (!_pcmPalyer) {
        //1:G.711编码；0:AAC编码；
        _pcmPalyer=[[BCAudioQueuePlay alloc] initWithAudioType:0];
    }
    return _pcmPalyer;
}
- (AACDecoder *)aacDecoder{
    if (!_aacDecoder) {
        _aacDecoder=[[AACDecoder alloc] init];
    }
    return _aacDecoder;
}
