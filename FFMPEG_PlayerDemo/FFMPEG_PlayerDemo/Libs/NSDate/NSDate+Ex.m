    ...
    if ([self isThisYear]) {
        if (self.isToday) {
            ...
            if (result <= 90) {
                ...
                return iFormatStr(NSLocalizedString(@"bc.other.minutes_ago",0),(int)(ceil(result/60.0)));
            }else{
            ...
            if ([NSDate is12HourFormat]) {
                fm.dateFormat = iFormatStr(@"'%@' hh:mm a",NSLocalizedString(@"bc.other.yesterday",0));
            }else{
                fm.dateFormat = iFormatStr(@"'%@' HH:mm",NSLocalizedString(@"bc.other.yesterday",0));
            }
        }else{
            if ([NSDate is12HourFormat]) {
                fm.dateFormat = @"MM-dd hh:mm a";
            }else{
...
-(NSString *)timeFormat5{
    ...
    fm.locale = [NSLocale systemLocale];
    return [fm stringFromDate:self];
}
